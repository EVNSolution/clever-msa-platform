...
- 현재 root는 whitelist에 포함된 `development/*` active target slice를 root-tracked source로 직접 소유한다.
- 루트를 새로 clone하거나 pull하면 active runtime source가 root checkout에 함께 포함된다.
- `git submodule update --init --recursive`는 더 이상 active workspace bootstrap 절차가 아니다.
- 구현 변경은 root worktree의 해당 `development/*` slice에서 수행하고, root는 repo map, platform docs, runtime source를 함께 관리한다.
...
| --- | --- | --- | --- | --- | --- |
| `runtime-prod-release` | runtime | active prod runtime release control plane | workload-based prod rollout, OIDC auth, resolved rollout plan, SSM dispatch owner. current minimal path validated on `EVDash-msa` | `development/runtime-prod-release/` | `migrated-target` |
...
| `front-driver-app` | front | driver-only mobile app child repo | React Native + Expo 기반 Android/iOS 배송원 native self-service 앱. 운영 원칙은 `native-only` | `development/front-driver-app/` | `empty-shell` |
...
- slice-local `AGENTS.md`는 모든 slice에 복제하지 않는다.
- 현재 유지 대상은 플랫폼 루트와 `edge-api-gateway`다.
- `AGENTS.md`와 각 `README.md`는 운영 안내 문서일 뿐 정본이 아니다.
- 나머지 slice의 canonical truth는 root `docs/`와 mapping 문서로 관리한다.
- 새 slice-local `AGENTS.md`는 경계 오염 위험이 높거나 접착 역할이 큰 slice에만 추가한다.

## Migration Priorities

1. `docs/`를 source of truth로 먼저 고정
2. local integration support repo 분리 완료
...
Superseded by the 2026-04-27 true monorepo umbrella decision.

## Governance Note (2026-04-27)

- Every active runtime unit exposed under the root `development/` whitelist is a root-tracked source slice.
- The root `clever-msa-platform` repository is the Git source of truth for those whitelisted slices.
- Service ownership boundaries remain enforced by docs, contracts, import rules, and deployment ownership rather than by separate child repositories.
- Root-level platform work should keep the whitelist stable and should not reintroduce support or legacy repos into the root Git tree by default.
- Historical child repo remotes may remain as archive or mirror references, but they are no longer the active source of truth for root-visible runtime code.
