...
- 현재 root는 whitelist에 포함된 `development/*` active target repo만 linked child repo로 노출한다.
...
- Every repo exposed under the root `development/` whitelist is an independent implementation repo.
- The root `clever-msa-platform` workspace is the umbrella GitHub view for those whitelisted repos, while runtime implementation ownership remains in each child repo.
- Root-level platform work should keep the whitelist stable and should not reintroduce support or legacy repos into the root GitHub tree by default.
- Active whitelisted child repos are represented as linked child repos from the root.
