...
- The root `clever-msa-platform` workspace is not the runtime source of truth for those repos.
- Root-level platform work should reference those repos through docs and mappings, not by newly tracking their code inside the root repo.
