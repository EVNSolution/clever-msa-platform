...
이 문서는 `ev-dashboard` current production deployment와 관련된 repo ownership, operator entrypoint, current runtime authority를 고정한다.

질문이 아래 중 하나면 이 문서를 먼저 본다.

- 어떤 repo가 build를 소유하는가
- 어떤 repo가 production release를 소유하는가
- 어떤 repo가 runtime inventory를 소유하는가
...
| `front-web-console` | production web console image owner. app repo는 image artifact를 만들지만 production release는 직접 수행하지 않는다 |
| active `service-*` repos | workload image owner. 각 repo는 자기 서비스 `main` push 기준 image artifact를 만든다. production deploy entrypoint는 아니다 |
| `front-driver-app` | current app-host production release lane 바깥의 client app repo. 현재 `evdash-msa` app host runtime inventory source of truth에는 포함되지 않는다 |
| root `clever-msa-platform` | docs, mappings, rollout truth owner only. production deploy workflow owner가 아니다 |

## Current Production Path

현재 production path는 아래 한 줄로 요약한다.

`repo main build -> ECR image artifact -> runtime-prod-release resolve -> SSM/app host reconcile -> runtime image map write-back -> post-release proof/evidence`

세부 규칙은 아래와 같다.

1. app repo와 service repo의 `main` push는 image artifact만 만든다.
...
- runtime inventory owner repo:
  `development/runtime-prod-platform`
- release control plane repo:
  `development/runtime-prod-release`
- public docs surface owner repo:
