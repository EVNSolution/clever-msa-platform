...
| `service-driver-operations-view` | driver operations read model, composed summary | account writes, driver profile writes, settlement writes | `service-driver-profile`, `service-account-access`, settlement services later |
...
| `service-region-registry` | region polygon, difficulty, region master | delivery execution truth, telemetry truth, route recommendation logic | organization and analytics later |
| `service-region-analytics` | region delivery statistics, region performance analytics | region master writes, telemetry ingest, planning truth writes | region registry and delivery/dispatch services later |
