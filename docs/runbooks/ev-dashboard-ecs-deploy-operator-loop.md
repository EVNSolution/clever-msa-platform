# ev-dashboard ECS Deploy Operator Loop

이 문서는 `ev-dashboard` ECS/CDK deploy가 왜 느리게 보이는지와, deploy 중 무엇을 기다리고 무엇을 디버깅해야 하는지를 한 화면에서 보게 하기 위한 runbook이다.

같은 요약은 `Deploy ev-dashboard ECS platform` workflow의 GitHub step summary에도 같이 적어 둔다. operator가 배포 화면 안에서 바로 같은 기준을 볼 수 있어야 한다.
...
4. 그 다음에만 `Deploy ev-dashboard ECS platform` workflow를 실행한다.
...
| `stateful quiet period` | `5-15m` | RDS create or update, new ECS services not visible yet | `60-90s` cadence로 기다린다 |
| `backend service registration` | `2-6m` | new ECS services appear with non-zero `desiredCount` | route bug로 단정하지 않는다 |
| `late gateway rollout` | `2-6m` | backends are healthy, but gateway still replacing tasks | mixed `200/502`는 잠시 허용 |
...
| --- | --- | --- |
| `502` and the new ECS service does not exist yet | backend is not created yet | wait |
| `502` and the backend exists but `edge-api-gateway` is still rolling | gateway replacement is not finished yet | wait, then check gateway logs only if it stays stuck |
...
- `60-90s` during RDS or other stateful create/update
- `60s` while waiting for new ECS services to appear
...
이 문서는 `ev-dashboard` ECS shared runtime operator loop만 다룬다.
