...
이 문서는 `CLEVER` 관리/운영 웹의 1차 working brand와 working canonical domain을 고정한다.

현재 목적은 “최종 브랜드를 영구 확정”하는 것이 아니라, 앞으로의 ingress, 문서, naming cleanup, 운영 커뮤니케이션에 쓸 기준 이름을 먼저 하나 정하는 것이다.

## Decision

1차 working canonical domain은 `clever-hub.com`으로 고정한다.

현재 시점에는 `clever-ops.com`은 메인 이름으로 쓰지 않는다.
...
`clever-hub.com`을 선택한 이유는 아래와 같다.
...
3. 나중에 이름을 바꾸더라도 현재 단계의 canonical working name으로 쓰기 좋다.
- ingress, docs, rollout, UI 타이틀에 일단 하나의 기준을 줄 수 있다.
- 최종 브랜드 확정 전까지 임시 기준으로 유지하기 쉽다.

## Why `clever-ops.com` Is Not Primary

`clever-ops.com`은 운영 도구처럼 들리는 장점이 있지만, 현재 메인 이름으로는 채택하지 않는다.

이유는 아래와 같다.

1. 제품 범위를 좁힌다.
- 현재 웹은 단순 ops screen보다 넓다.

2. 장기 제품명으로는 다소 딱딱하다.
- 대내 운영 화면에는 어울리지만 대외 제품명이나 대표 포털명으로는 확장성이 떨어질 수 있다.

3. 이후 `hub`를 대표 이름으로 두는 구조가 ingress와 UI naming에 더 자연스럽다.
...
- 도메인 결정 spec과 이후 ingress spec에서 `clever-hub.com`을 primary candidate로 취급한다.
- 공개 경로 정식화 논의에서 대표 FQDN은 `clever-hub.com` 기준으로 설계한다.
...
1. 공개 진입점 정식화
- `ALB + ACM + Route53` 설계를 `clever-hub.com` 기준으로 진행
...
- primary working domain: `clever-hub.com`
