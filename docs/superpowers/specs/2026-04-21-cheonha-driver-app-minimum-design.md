...
1. 체크한 경우에만 앱 재실행 후 세션 복구를 시도한다.
2. 체크하지 않으면 앱 종료 후 다시 로그인해야 한다.
...
2. 1차에서는 로그인 수단이 아니다.
3. 기존 `phone_credential`로 저장하지 않는다.
4. 즉 `전화번호 로그인 방식`과 `가입 연락처 전화번호`는 같은 개념으로 취급하지 않는다.
...
2. system admin은 빈 메인 화면과 공통 `MY`만 사용한다.
3. 천하운수 `driver_account_create`는 server setting 기준 auto-approve 대상으로 유지한다.
4. 가입 연락처 전화번호는 auth login method가 아니라 profile/contact field로 저장한다.
