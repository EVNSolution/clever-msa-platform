...
서브도메인 1차 MVP의 상단 구조는 아래 3개로 고정한다.

- `대시보드`
- `배차`
- `정산`

이 구조가 1차 MVP의 핵심 제품 surface다.
...
`정산` 탭은 자동 계산 결과를 보는 업무 공간이다.

핵심 원칙:

- 계산을 시작하는 화면이 아니다
- 이미 자동화된 결과를 조회하고 잠그는 공간이다
...
3. backend/API/자동화 기능은 우리 current runtime truth를 따른다
...
`배차`는 단순 파일 업로드 기능이 아니다. 업로드는 아래 자동화의 출발점이어야 한다.
...
3. 서브도메인 탑바가 `대시보드 / 배차 / 정산`으로 고정된다
4. `배차 업로드` 후 `금일 배차 / 금월 근태 / 정산 초안`까지 이어지는 흐름을 검증한다
