...
이 문서는 현재 dev에서 사용 중인 임시 공개 경로(`EC2 public IP + 8080`)를 정식 ingress 구조로 전환하기 위한 1차 설계를 고정한다.
...
- 외부 접근은 EC2 public IP와 임시 `8080` inbound 개방으로만 확인 중이다.
- gateway는 이미 존재하고 `/`와 `/api/*`를 내부 라우팅한다.
- ALB는 없다.
- ACM 인증서도 없다.
- Route53 연결도 없다.

즉, 서비스 자체는 동작하지만 ingress는 아직 운영용 구조가 아니다.
...
- `https://clever-hub.com/`
  - gateway가 현재 프론트 runtime으로 reverse proxy
- `https://clever-hub.com/api/...`
...
1차는 `clever-hub.com` 단일 도메인 기준으로 ACM public certificate를 발급한다.
...
1차 canonical public name은 `clever-hub.com`이다.
