...
2. `front-web-console`에 **dev/local-test 전용** 세션 주입 route를 둔다.
...
- route는 dev/local-test에서만 활성화된다.
...
- 완료 후 `/`로 이동한다.
- `세션 초기화` 버튼은 저장 세션을 지우고 로그인 상태를 비운다.
...
- dev/local-test에서만 활성화
- 실제 `/api` 프록시 대신 mock repository가 응답
...
2. dev/local-test 이외 환경에서는 route 비활성
3. host별 preset 고정
