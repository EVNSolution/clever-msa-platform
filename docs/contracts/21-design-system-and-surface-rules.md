...
4. mobile appendix도 같은 font 축을 우선 검토하되, 실제 native app font loading/linking 방식은 Expo bootstrap 검증 후 확정한다.
...
- framework 상태: `React Native + Expo`
- 운영 원칙: `native-only`, `web non-target`, `Expo Go non-target`
...
4. exact component composition과 design token export 방식은 `React Native + Expo` 기준으로 후속 확정한다.
