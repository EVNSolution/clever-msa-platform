...
- `front-driver-app`은 active child repo로 생성되었다.
...
4. mobile appendix도 같은 font 축을 우선 검토하되, 실제 Expo / React Native font 세팅은 `확인 필요`다.
...
- 현재 상태: active child repo
- 기술 방향: `Expo` (`React Native + TypeScript`)
...
4. exact Expo component composition과 design token export 방식은 `확인 필요`다.

### 확인 필요

- Expo / React Native font 설정 방식
