...
1. settlement 그룹 상단에는 현재 단계와 다음 단계를 한눈에 볼 수 있는 단계 네비게이션을 둔다.
2. 단계 순서는 `정산 조회 -> 정산 기준 -> 정산 입력 -> 정산 실행 -> 정산 결과`로 고정한다.
3. 각 단계는 짧은 설명을 같이 보여서 페이지를 옮겨도 맥락이 끊기지 않게 한다.
4. 현재 단계에서는 `이전 단계`, `다음 단계` 이동을 같이 제공한다.
5. overview는 정산 그룹의 진입점이자 현재 진행 상태를 요약해 주는 landing page로 본다.
