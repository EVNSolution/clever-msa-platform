from datetime import datetime, timedelta, timezone
from decimal import Decimal
from uuid import uuid4
from unittest.mock import MagicMock, patch

import jwt
from django.conf import settings
from django.db import IntegrityError
from django.test import TestCase, TransactionTestCase
from rest_framework.test import APIClient

from settlements.services.source_clients import SourceNotFoundError


class SettlementApiTests(TestCase):
    def setUp(self) -> None:
        self.client = APIClient()
        self.admin_token = self._issue_token("admin")
        self.user_token = self._issue_token("user")

    def _issue_token(self, role: str) -> str:
        now = datetime.now(timezone.utc)
        payload = {
            "sub": str(uuid4()),
            "email": f"{role}@example.com",
            "role": role,
            "iss": settings.JWT_ISSUER,
            "aud": settings.JWT_AUDIENCE,
            "iat": int(now.timestamp()),
            "exp": int((now + timedelta(hours=1)).timestamp()),
            "jti": str(uuid4()),
            "type": "access",
        }
        return jwt.encode(payload, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)

    def _authenticate(self, token: str) -> None:
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")

    def _run_payload(self):
        return {
            "company_id": str(uuid4()),
            "fleet_id": str(uuid4()),
            "period_start": "2026-03-01",
            "period_end": "2026-03-31",
            "status": "draft",
        }

    def _item_payload(self, settlement_run_id: str):
        return {
            "settlement_run_id": settlement_run_id,
            "driver_id": str(uuid4()),
            "amount": "125000.50",
            "payout_status": "pending",
        }

    def _stub_source_clients(
        self,
        mock_source_clients,
        *,
        company_id: str = "30000000-0000-0000-0000-000000000001",
        fleet_id: str = "40000000-0000-0000-0000-000000000001",
        snapshots: list[dict] | None = None,
        attendance_days: list[dict] | None = None,
        driver_day_exceptions: list[dict] | None = None,
    ):
        source_client = MagicMock()
        source_client.get_company_fleet_pricing_table.return_value = {
            "pricing_table_id": str(uuid4()),
            "company_id": company_id,
            "fleet_id": fleet_id,
            "box_sale_unit_price": "1000.00",
            "box_purchase_unit_price": "800.00",
            "overtime_fee": "20000.00",
        }
        source_client.list_active_daily_snapshots.return_value = snapshots or []
        source_client.bulk_lookup_attendance_days.return_value = attendance_days or []
        source_client.list_driver_day_exceptions.return_value = driver_day_exceptions or []
        mock_source_clients.return_value = source_client
        return source_client

    def test_health_endpoint_responds(self):
        response = self.client.get("/health/")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["status"], "ok")

    @patch("settlements.views.SourceClients", create=True)
    def test_admin_can_crud_settlement_run_and_item(self, mock_source_clients):
        self._stub_source_clients(mock_source_clients)
        self._authenticate(self.admin_token)

        run_response = self.client.post("/runs/", self._run_payload(), format="json")
        self.assertEqual(run_response.status_code, 201)
        settlement_run_id = run_response.data["settlement_run_id"]

        item_response = self.client.post(
            "/items/",
            self._item_payload(settlement_run_id),
            format="json",
        )
        self.assertEqual(item_response.status_code, 201)
        settlement_item_id = item_response.data["settlement_item_id"]

        self.assertEqual(self.client.get(f"/runs/{settlement_run_id}/").status_code, 200)
        self.assertEqual(self.client.get(f"/items/{settlement_item_id}/").status_code, 200)
        self.assertEqual(
            self.client.patch(
                f"/runs/{settlement_run_id}/",
                {"status": "closed"},
                format="json",
            ).status_code,
            200,
        )
        self.assertEqual(
            self.client.patch(
                f"/items/{settlement_item_id}/",
                {"payout_status": "paid"},
                format="json",
            ).status_code,
            200,
        )
        self.assertEqual(self.client.delete(f"/items/{settlement_item_id}/").status_code, 204)
        self.assertEqual(self.client.delete(f"/runs/{settlement_run_id}/").status_code, 204)

    @patch("settlements.views.SourceClients", create=True)
    def test_settlement_run_reads_company_fleet_pricing_table(self, mock_source_clients):
        self._stub_source_clients(
            mock_source_clients,
            snapshots=[
            {
                "daily_delivery_input_snapshot_id": str(uuid4()),
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "driver_id": "10000000-0000-0000-0000-000000000001",
                "service_date": "2026-03-15",
                "delivery_count": 133,
                "total_distance_km": "0.00",
                "total_base_amount": "0.00",
                "source_record_count": 1,
                "status": "active",
            }
            ],
            attendance_days=[
                {
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "attendance_date": "2026-03-15",
                    "final_status": "worked",
                }
            ],
        )

        self._authenticate(self.admin_token)

        response = self.client.post(
            "/runs/",
            {
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "period_start": "2026-03-01",
                "period_end": "2026-03-31",
                "status": "draft",
            },
            format="json",
        )

        self.assertEqual(response.status_code, 201)
        self.assertEqual(len(response.data["items"]), 1)
        self.assertEqual(
            response.data["items"][0]["driver_id"],
            "10000000-0000-0000-0000-000000000001",
        )
        self.assertEqual(Decimal(response.data["items"][0]["amount"]), Decimal("106400.00"))

    @patch("settlements.views.SourceClients", create=True)
    def test_settlement_run_keeps_box_count_times_unit_price_formula_even_on_special_day(self, mock_source_clients):
        self._stub_source_clients(
            mock_source_clients,
            snapshots=[
                {
                    "daily_delivery_input_snapshot_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "service_date": "2026-03-15",
                    "delivery_count": 10,
                    "total_distance_km": "0.00",
                    "total_base_amount": "0.00",
                    "source_record_count": 1,
                    "status": "active",
                }
            ],
            attendance_days=[
                {
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "attendance_date": "2026-03-15",
                    "final_status": "worked",
                }
            ],
            driver_day_exceptions=[
                {
                    "driver_day_exception_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "dispatch_date": "2026-03-15",
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "work_rule": {"system_kind": "overtime"},
                },
                {
                    "driver_day_exception_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "dispatch_date": "2026-03-15",
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "work_rule": {"system_kind": "day_off"},
                },
                {
                    "driver_day_exception_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "dispatch_date": "2026-03-16",
                    "driver_id": "10000000-0000-0000-0000-000000000002",
                    "work_rule": {"system_kind": "overtime"},
                },
            ],
        )

        self._authenticate(self.admin_token)

        response = self.client.post(
            "/runs/",
            {
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "period_start": "2026-03-01",
                "period_end": "2026-03-31",
                "status": "draft",
            },
            format="json",
        )

        self.assertEqual(response.status_code, 201)
        self.assertEqual(len(response.data["items"]), 1)
        self.assertEqual(
            Decimal(response.data["items"][0]["amount"]),
            Decimal("8000.00"),
        )

    @patch("settlements.views.SourceClients", create=True)
    def test_settlement_run_excludes_non_worked_attendance_days(self, mock_source_clients):
        self._stub_source_clients(
            mock_source_clients,
            snapshots=[
                {
                    "daily_delivery_input_snapshot_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "service_date": "2026-03-15",
                    "delivery_count": 10,
                    "total_distance_km": "0.00",
                    "total_base_amount": "0.00",
                    "source_record_count": 1,
                    "status": "active",
                },
                {
                    "daily_delivery_input_snapshot_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "driver_id": "10000000-0000-0000-0000-000000000002",
                    "service_date": "2026-03-16",
                    "delivery_count": 15,
                    "total_distance_km": "0.00",
                    "total_base_amount": "0.00",
                    "source_record_count": 1,
                    "status": "active",
                },
            ],
            attendance_days=[
                {
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "attendance_date": "2026-03-15",
                    "final_status": "worked",
                },
                {
                    "driver_id": "10000000-0000-0000-0000-000000000002",
                    "attendance_date": "2026-03-16",
                    "final_status": "day_off",
                },
            ],
        )

        self._authenticate(self.admin_token)

        response = self.client.post(
            "/runs/",
            {
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "period_start": "2026-03-01",
                "period_end": "2026-03-31",
                "status": "draft",
            },
            format="json",
        )

        self.assertEqual(response.status_code, 201)
        self.assertEqual(len(response.data["items"]), 1)
        self.assertEqual(response.data["items"][0]["driver_id"], "10000000-0000-0000-0000-000000000001")

    @patch("settlements.views.SourceClients", create=True)
    def test_settlement_run_rejects_missing_attendance_truth(self, mock_source_clients):
        self._stub_source_clients(
            mock_source_clients,
            snapshots=[
                {
                    "daily_delivery_input_snapshot_id": str(uuid4()),
                    "company_id": "30000000-0000-0000-0000-000000000001",
                    "fleet_id": "40000000-0000-0000-0000-000000000001",
                    "driver_id": "10000000-0000-0000-0000-000000000001",
                    "service_date": "2026-03-15",
                    "delivery_count": 10,
                    "total_distance_km": "0.00",
                    "total_base_amount": "0.00",
                    "source_record_count": 1,
                    "status": "active",
                }
            ],
            attendance_days=[],
        )

        self._authenticate(self.admin_token)

        response = self.client.post(
            "/runs/",
            {
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "period_start": "2026-03-01",
                "period_end": "2026-03-31",
                "status": "draft",
            },
            format="json",
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["code"], "validation_error")

    @patch("settlements.views.SourceClients", create=True)
    def test_settlement_run_rejects_missing_company_fleet_pricing_table(self, mock_source_clients):
        source_client = MagicMock()
        source_client.get_company_fleet_pricing_table.side_effect = SourceNotFoundError(
            "Company/fleet pricing table not found."
        )
        source_client.list_active_daily_snapshots.return_value = []
        source_client.list_driver_day_exceptions.return_value = []
        mock_source_clients.return_value = source_client

        self._authenticate(self.admin_token)

        response = self.client.post(
            "/runs/",
            {
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "period_start": "2026-03-01",
                "period_end": "2026-03-31",
                "status": "draft",
            },
            format="json",
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["code"], "validation_error")
        self.assertEqual(response.data["details"]["detail"][0], "Company/fleet pricing table not found.")

    def test_admin_cannot_create_settlement_run_with_invalid_status(self):
        self._authenticate(self.admin_token)

        payload = self._run_payload()
        payload["status"] = "processing"

        response = self.client.post("/runs/", payload, format="json")

        self.assertEqual(response.status_code, 400)
        self.assertEqual(set(response.data.keys()), {"code", "message", "details"})
        self.assertIn("status", response.data["details"])

    @patch("settlements.views.SourceClients", create=True)
    def test_user_can_read_settlement_resources(self, mock_source_clients):
        self._stub_source_clients(mock_source_clients)
        self._authenticate(self.admin_token)
        self.assertEqual(self.client.post("/runs/", self._run_payload(), format="json").status_code, 201)

        self._authenticate(self.user_token)

        runs_response = self.client.get("/runs/")
        items_response = self.client.get("/items/")

        self.assertEqual(runs_response.status_code, 200)
        self.assertEqual(items_response.status_code, 200)

    @patch("settlements.views.SourceClients", create=True)
    def test_user_can_read_driver_daily_settlements(self, mock_source_clients):
        source_client = MagicMock()
        source_client.list_driver_daily_snapshots.return_value = [
            {
                "daily_delivery_input_snapshot_id": str(uuid4()),
                "company_id": "30000000-0000-0000-0000-000000000001",
                "fleet_id": "40000000-0000-0000-0000-000000000001",
                "driver_id": "10000000-0000-0000-0000-000000000001",
                "service_date": "2026-04-17",
                "delivery_count": 12,
                "status": "active",
            }
        ]
        source_client.get_company_fleet_pricing_table.return_value = {
            "box_purchase_unit_price": "4700.00",
        }
        source_client.bulk_lookup_attendance_days.return_value = [
            {
                "driver_id": "10000000-0000-0000-0000-000000000001",
                "attendance_date": "2026-04-17",
                "final_status": "worked",
            }
        ]
        source_client.list_driver_day_exceptions.return_value = []
        mock_source_clients.return_value = source_client

        self._authenticate(self.user_token)

        response = self.client.get(
            "/drivers/10000000-0000-0000-0000-000000000001/daily-settlements/?date_from=2026-04-01&date_to=2026-04-30"
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["driver_id"], "10000000-0000-0000-0000-000000000001")
        self.assertEqual(response.data["summary"]["regular_days"], 1)
        self.assertEqual(response.data["summary"]["total_amount"], "56400.00")
        self.assertEqual(response.data["results"][0]["unit_price"], "4700.00")
        self.assertEqual(response.data["results"][0]["total_amount"], "56400.00")

    @patch("settlements.views.SourceClients", create=True)
    def test_driver_daily_settlements_rejects_invalid_date_window(self, mock_source_clients):
        self._authenticate(self.user_token)

        response = self.client.get(
            "/drivers/10000000-0000-0000-0000-000000000001/daily-settlements/?date_from=2026-04-30&date_to=2026-04-01"
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["code"], "validation_error")
        self.assertIn("date_to", response.data["details"])
        mock_source_clients.assert_not_called()

    @patch("settlements.views.SourceClients", create=True)
    def test_driver_daily_settlements_maps_dependency_outage_to_503(self, mock_source_clients):
        source_client = MagicMock()
        source_client.list_driver_daily_snapshots.side_effect = SourceNotFoundError("snapshot scope unavailable")
        mock_source_clients.return_value = source_client

        self._authenticate(self.user_token)

        response = self.client.get(
            "/drivers/10000000-0000-0000-0000-000000000001/daily-settlements/?date_from=2026-04-01&date_to=2026-04-30"
        )

        self.assertEqual(response.status_code, 503)
        self.assertEqual(response.data["code"], "upstream_service_unavailable")

    @patch("settlements.views.SourceClients", create=True)
    def test_user_can_filter_settlement_reads_by_company_and_fleet(self, mock_source_clients):
        self._authenticate(self.admin_token)

        company_id = str(uuid4())
        fleet_id = str(uuid4())
        other_company_id = str(uuid4())
        other_fleet_id = str(uuid4())

        self._stub_source_clients(
            mock_source_clients,
            company_id=company_id,
            fleet_id=fleet_id,
        )

        run_a = self.client.post(
            "/runs/",
            {
                **self._run_payload(),
                "company_id": company_id,
                "fleet_id": fleet_id,
            },
            format="json",
        )
        self.assertEqual(run_a.status_code, 201)

        run_b = self.client.post(
            "/runs/",
            {
                **self._run_payload(),
                "company_id": other_company_id,
                "fleet_id": other_fleet_id,
            },
            format="json",
        )
        self.assertEqual(run_b.status_code, 201)

        item_a = self.client.post(
            "/items/",
            self._item_payload(run_a.data["settlement_run_id"]),
            format="json",
        )
        self.assertEqual(item_a.status_code, 201)

        item_b = self.client.post(
            "/items/",
            self._item_payload(run_b.data["settlement_run_id"]),
            format="json",
        )
        self.assertEqual(item_b.status_code, 201)

        self._authenticate(self.user_token)

        runs_response = self.client.get(
            f"/runs/?company_id={company_id}&fleet_id={fleet_id}",
        )
        items_response = self.client.get(
            f"/items/?company_id={company_id}&fleet_id={fleet_id}",
        )

        self.assertEqual(runs_response.status_code, 200)
        self.assertEqual(items_response.status_code, 200)
        self.assertEqual(len(runs_response.data), 1)
        self.assertEqual(len(items_response.data), 1)
        self.assertEqual(runs_response.data[0]["settlement_run_id"], run_a.data["settlement_run_id"])
        self.assertEqual(items_response.data[0]["settlement_item_id"], item_a.data["settlement_item_id"])

    def test_list_filter_rejects_invalid_uuid(self):
        self._authenticate(self.user_token)

        response = self.client.get("/runs/?company_id=not-a-uuid&fleet_id=also-not-a-uuid")

        self.assertEqual(response.status_code, 400)
        self.assertEqual(set(response.data.keys()), {"code", "message", "details"})
        self.assertIn("company_id", response.data["details"])

    def test_user_cannot_write_settlement_resources(self):
        self._authenticate(self.user_token)

        run_response = self.client.post("/runs/", self._run_payload(), format="json")
        item_response = self.client.post("/items/", self._item_payload(str(uuid4())), format="json")

        self.assertEqual(run_response.status_code, 403)
        self.assertEqual(set(run_response.data.keys()), {"code", "message", "details"})
        self.assertEqual(item_response.status_code, 403)
        self.assertEqual(set(item_response.data.keys()), {"code", "message", "details"})

    def test_payroll_routes_do_not_expose_legacy_settlement_engine_endpoints(self):
        self._authenticate(self.admin_token)

        for path in (
            "/daily-settlement/",
            "/monthly-settlement/",
            "/group-settlement/",
            "/settlement-policy/",
        ):
            response = self.client.get(path)
            self.assertEqual(response.status_code, 404)

    def test_unauthenticated_read_returns_401_shape(self):
        response = self.client.get("/runs/")

        self.assertEqual(response.status_code, 401)
        self.assertEqual(set(response.data.keys()), {"code", "message", "details"})

    def test_missing_resource_returns_404_shape(self):
        self._authenticate(self.admin_token)

        response = self.client.get(f"/runs/{uuid4()}/")

        self.assertEqual(response.status_code, 404)
        self.assertEqual(set(response.data.keys()), {"code", "message", "details"})


class SettlementItemDatabaseConstraintTests(TransactionTestCase):
    reset_sequences = True

    def test_settlement_item_requires_an_existing_settlement_run_at_the_database_layer(self):
        with self.assertRaises(IntegrityError):
            from settlements.models import SettlementItem

            SettlementItem.objects.create(
                settlement_run_id=uuid4(),
                driver_id=uuid4(),
                amount="125000.50",
                payout_status="pending",
            )
