...
class ManagerAccountFleetAssignment(models.Model):
    manager_account_fleet_assignment_id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False,
    )
    ...
    def clean(self):
        if self.manager_account_id and self.company_id and self.manager_account.company_id != self.company_id:
            raise ValidationError(
                {"company_id": "Fleet assignment company must match manager account company."}
            )
