...
class SignupIntakeApiTests(TestCase):
    ...
    def test_signup_request_intake_creates_identity_credentials_consents_and_request(self):
        ...
        self.assertTrue(check_password("Signup12!", password_credential.password_hash))
    ...
    @override_settings(
        AUTO_APPROVED_DRIVER_SIGNUP_COMPANY_IDS=["30000000-0000-0000-0000-000000000001"]
    )
    def test_signup_request_intake_auto_approves_driver_request_and_creates_driver_account(self):
        response = self.client.post(
            "/identity-signup-requests/",
            {
                "name": "배송원즉시",
                "birth_date": "1991-02-03",
                "email": "driver-direct@example.com",
                "password": "Signup12!",
                "company_id": "30000000-0000-0000-0000-000000000001",
                "request_types": [
        ...
        self.assertEqual(len(response.data["requests"]), 1)
        self.assertEqual(
            response.data["requests"][0]["status"],
            IdentitySignupRequest.Status.APPROVED,
        )

        identity = Identity.objects.get(identity_id=response.data["identity_id"])
        request = IdentitySignupRequest.objects.get(
            identity=identity,
        ...
        driver_account = DriverAccount.objects.get(identity=identity, status=DriverAccount.Status.ACTIVE)
        self.assertEqual(str(driver_account.company_id), "30000000-0000-0000-0000-000000000001")
        self.assertTrue(
    ...
    def test_signup_request_intake_supports_creating_both_manager_and_driver_requests(self):
        ...
        self.assertEqual(
            request_statuses[IdentitySignupRequest.RequestType.MANAGER_ACCOUNT_CREATE],
            IdentitySignupRequest.Status.PENDING,
        )
        self.assertEqual(
            request_statuses[IdentitySignupRequest.RequestType.DRIVER_ACCOUNT_CREATE],
            IdentitySignupRequest.Status.APPROVED,
        )
        self.assertEqual(
            set(
        ...
        identity = Identity.objects.get(identity_id=response.data["identity_id"])
        self.assertTrue(
            DriverAccount.objects.filter(identity=identity, status=DriverAccount.Status.ACTIVE).exists()
        )

    @override_settings(
        AUTO_APPROVED_DRIVER_SIGNUP_COMPANY_IDS=["30000000-0000-0000-0000-000000000001"]
    )
    def test_signup_request_intake_keeps_other_company_driver_request_pending(self):
        response = self.client.post(
            "/identity-signup-requests/",
            {
                "name": "타회사배송원",
                "birth_date": "1992-03-04",
                "email": "other-company-driver@example.com",
                "password": "Signup12!",
    ...
    def test_signup_request_intake_rejects_phone_signup(self):
        response = self.client.post(
            "/identity-signup-requests/",
            {
                "name": "전화배송원",
        ...
        self.assertFalse(Identity.objects.filter(name="전화배송원").exists())
    ...
    def test_signup_request_intake_rejects_weak_password(self):
        response = self.client.post(
            "/identity-signup-requests/",
            {
                "name": "약한비밀번호",
        ...
        self.assertFalse(Identity.objects.filter(name="약한비밀번호").exists())
...
class IdentityRequestApiTests(TestCase):
    ...
    def test_identity_user_can_create_new_driver_request_for_selected_company(self):
        ...
        self.assertEqual(response.data["status"], IdentitySignupRequest.Status.APPROVED)
        driver_account = DriverAccount.objects.get(identity=identity, status=DriverAccount.Status.ACTIVE)
...
class ManagerAccountManagementApiTests(TestCase):
    ...
    def test_company_super_admin_can_change_lower_manager_role_in_place(self):
        ...
        with patch(
            "accounts.services.organization_fleet_lookup_client.OrganizationFleetLookupClient.get_fleet_company_id",
        ...
        self.assertEqual(
            list(
                ManagerAccountFleetAssignment.objects.filter(
                    manager_account=lower_account,
                ).values_list("fleet_id", flat=True)
            ),
            [uuid.UUID("70000000-0000-0000-0000-000000000001")],
        )

    def test_fleet_role_change_requires_fleet_ids(self):
        super_identity = self._create_identity_with_password(
            name="역할 전환 총괄",
            birth_date="1980-01-01",
            email="change-role-super-missing@example.com",
            password="change-role-super-missing-pass-123",
        )
        lower_identity = self._create_identity_with_password(
            name="역할 전환 대상",
            birth_date="1990-01-02",
            email="change-role-target-missing@example.com",
            password="change-role-target-missing-pass-123",
        )
        self._create_manager_account(
            identity=super_identity,
            company_id=self.company_id,
        ...
        self._login_identity("change-role-super-missing@example.com", "change-role-super-missing-pass-123")
        response = self.client.post(
            f"/manager-accounts/{lower_account.manager_account_id}/change-role/",
            {"role_type": ManagerAccount.RoleType.FLEET_MANAGER},
            format="json",
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["fleet_ids"], ["Fleet-scoped roles require at least one fleet assignment."])
    ...
    def test_company_super_admin_can_complete_manager_setup_as_fleet_manager(self):
        ...
        with patch(
            ...
            response = self.client.post(
                ...
                {
                    "role_type": ManagerAccount.RoleType.FLEET_MANAGER,
                    "fleet_ids": ["70000000-0000-0000-0000-000000000002"],
                },
                format="json",
            )
        ...
        self.assertEqual(
            list(
                ManagerAccountFleetAssignment.objects.filter(
                    manager_account=created_manager,
                ).values_list("fleet_id", flat=True)
            ),
            [uuid.UUID("70000000-0000-0000-0000-000000000002")],
        )

    def test_company_role_setup_rejects_fleet_ids(self):
        super_identity = self._create_identity_with_password(
            name="회사 레벨 설정 총괄",
            birth_date="1980-01-01",
            email="setup-company-super@example.com",
            password="setup-company-super-pass-123",
        )
        request_identity = self._create_identity_with_password(
            name="회사 레벨 설정 신청자",
            birth_date="1990-01-02",
            email="setup-company-target@example.com",
            password="setup-company-target-pass-123",
        )
        self._create_manager_account(
            identity=super_identity,
            company_id=self.company_id,
        ...
        request = IdentitySignupRequest.objects.create(
            ...
            company_id=self.company_id,
            request_type=IdentitySignupRequest.RequestType.MANAGER_ACCOUNT_CREATE,
            status=IdentitySignupRequest.Status.AWAITING_SETUP,
        )

        self._login_identity("setup-company-super@example.com", "setup-company-super-pass-123")
        response = self.client.post(
            f"/identity-signup-requests/{request.identity_signup_request_id}/complete-setup/",
            {
                "role_type": ManagerAccount.RoleType.SETTLEMENT_MANAGER,
                "fleet_ids": ["70000000-0000-0000-0000-000000000003"],
            },
            format="json",
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["fleet_ids"], ["Company-scoped roles cannot receive fleet assignments."])
...
class IdentityLoginMethodApiTests(TestCase):
    ...
    def test_identity_login_supports_identifier_password_with_phone_number(self):
        identity = Identity.objects.create(name="전화 로그인 사용자", birth_date="1990-01-02")
        now = timezone.now()
        login_method = IdentityLoginMethod.objects.create(
            identity=identity,
            method_type=IdentityLoginMethod.MethodType.PHONE,
        ...
        PasswordCredential.objects.create(
            identity=identity,
            password_hash=make_password("PhoneLogin12!"),
        )
        IdentityConsentCurrent.objects.create(
            identity=identity,
            privacy_policy_version="v1.0",
        ...
        response = self.client.post(
            "/identity-login/",
            {"identifier": "01012341234", "password": "PhoneLogin12!"},
            format="json",
        )
    ...
    def test_identity_user_can_change_shared_password(self):
        ...
        succeeded_login = fresh_client.post(
            "/identity-login/",
            {"email": "password-owner@example.com", "password": "AfterPassword12!"},
            format="json",
        )
    ...
    def test_identity_password_change_rejects_weak_new_password(self):
        self._create_identity_with_password(
            name="약한 변경 사용자",
            birth_date="1990-01-02",
            email="weak-password-owner@example.com",
            password="BeforePassword12!",
        )
        self._login_identity("weak-password-owner@example.com", "BeforePassword12!")

        response = self.client.put(
            "/identity-password/",
            {
                "current_password": "BeforePassword12!",
    ...
    def test_identity_recovery_rejects_weak_password(self):
        identity = self._create_identity_with_password(
            name="약한 복구 대상",
            birth_date="1990-01-02",
            email="before-weak-recovery@example.com",
            password="BeforeRecovery12!",
        )
        identity.status = Identity.Status.ARCHIVED
        identity.archived_at = timezone.now()
        identity.save(update_fields=["status", "archived_at"])
        ...
        response = self.client.post(
            "/identity-recovery/",
            {
                "name": "약한 복구 대상",
                "birth_date": "1990-01-02",
                "email": "after-weak-recovery@example.com",
