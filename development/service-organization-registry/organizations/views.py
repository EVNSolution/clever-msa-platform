...
from organizations.serializers import (
    ...
    PublicCompanyResolveSerializer,
)
...
class CompanyViewSet(PublicRefLookupMixin, viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == "public_resolve":
            return PublicCompanyResolveSerializer
    ...
    @action(
        ...
        authentication_classes=[],
        permission_classes=[permissions.AllowAny],
        url_path="public/resolve",
    )
    def public_resolve(self, request):
        ...
        company = (
            self.get_queryset()
            .exclude(tenant_code__isnull=True)
            .exclude(tenant_code="")
            .exclude(workflow_profile="")
            .filter(tenant_code=tenant_code)
            .first()
        )
        if company is None:
            raise NotFound("Requested company cockpit was not found.")
