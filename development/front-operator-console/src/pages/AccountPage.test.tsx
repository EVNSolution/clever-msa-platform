...
describe('Operator AccountPage', () => {
  it('supports profile save and request create/cancel for self-service', async () => {
    ...
    await screen.findByDisplayValue('운영자');
    fireEvent.change(screen.getByLabelText('이름'), { target: { value: '변경된 운영자' } });
    ...
    fireEvent.change(screen.getByLabelText('요청 종류'), {
      target: { value: 'manager_account_create' },
    });
