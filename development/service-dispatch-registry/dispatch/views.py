...
from dispatch.serializers import (
    ...
    DispatchUploadBatchSerializer,
    DispatchUploadPreviewRequestSerializer,
...
from dispatch.services.dispatch_upload_service import DispatchUploadService
...
class DispatchUploadPreviewView(APIView):
    permission_classes = [AuthenticatedReadAdminWrite]

    def post(self, request):
        require_nav_access(request, "dispatch")
        serializer = DispatchUploadPreviewRequestSerializer(data=request.data, context={"request": request})
        serializer.is_valid(raise_exception=True)
        batch = DispatchUploadService().preview_upload(
            dispatch_plan=serializer.validated_data.get("dispatch_plan"),
        ...
        return Response(DispatchUploadBatchSerializer(batch).data, status=status.HTTP_200_OK)


class DispatchUploadConfirmView(APIView):
    permission_classes = [AuthenticatedReadAdminWrite]

    def post(self, request, upload_batch_id):
        require_nav_access(request, "dispatch")
        upload_batch = generics.get_object_or_404(
            DispatchUploadBatch.objects.prefetch_related("rows"),
            upload_batch_id=upload_batch_id,
        )
        upload_batch = DispatchUploadService().confirm_upload(
            upload_batch=upload_batch,
            authorization=request.headers.get("Authorization", ""),
        )
        return Response(DispatchUploadBatchSerializer(upload_batch).data, status=status.HTTP_200_OK)


class DispatchUploadBatchListView(generics.ListAPIView):
    queryset = DispatchUploadBatch.objects.prefetch_related("rows", "dispatch_plan").all()
    serializer_class = DispatchUploadBatchSerializer
    permission_classes = [AuthenticatedReadAdminWrite]

    def get_queryset(self):
        queryset = super().get_queryset()
        auth_payload = getattr(self.request, "auth", None)
        session_company_id = None
        if isinstance(auth_payload, dict) and auth_payload.get("active_account_type") == "manager":
            session_company_id = auth_payload.get("company_id")

        company_id = session_company_id or self.request.query_params.get("company_id")
        fleet_id = self.request.query_params.get("fleet_id")
        dispatch_date = self.request.query_params.get("dispatch_date")
