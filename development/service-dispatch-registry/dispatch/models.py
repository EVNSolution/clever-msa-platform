...
class DispatchUploadBatch(models.Model):
    ...
    upload_batch_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    dispatch_plan = models.ForeignKey(
        DispatchPlan,
    ...
    upload_status = models.CharField(max_length=32, choices=UploadStatus.choices, default=UploadStatus.DRAFT)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        ordering = ("upload_batch_id",)

    def clean(self):
        errors = {}
        if self.dispatch_plan_id:
            if self.company_id != self.dispatch_plan.company_id:
                errors["company_id"] = "company_id must match dispatch_plan.company_id."
            if self.fleet_id != self.dispatch_plan.fleet_id:
                errors["fleet_id"] = "fleet_id must match dispatch_plan.fleet_id."
            if self.dispatch_date != self.dispatch_plan.dispatch_date:
...
class DispatchUploadRow(models.Model):
    upload_row_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    class Meta:
        ordering = ("upload_batch_id", "row_index", "upload_row_id")
