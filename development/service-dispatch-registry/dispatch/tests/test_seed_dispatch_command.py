...
class SeedDispatchCommandTests(TestCase):
    def test_seed_command_creates_deterministic_dispatch_data(self):
        ...
        upload_batch = models_module.DispatchUploadBatch.objects.get(
            upload_batch_id=seed_module.SAMPLE_UPLOAD_BATCH_ID
        )
        ...
        self.assertEqual(upload_batch.company_id, seed_module.SAMPLE_COMPANY_ID)
        self.assertEqual(upload_batch.fleet_id, seed_module.SAMPLE_FLEET_ID)
        self.assertEqual(upload_batch.dispatch_date, date(2026, 3, 24))
        self.assertEqual(
