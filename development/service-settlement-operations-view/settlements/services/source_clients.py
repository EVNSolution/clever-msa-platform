...
class SourceClients:
    ...
    def list_settlement_runs(self, *, authorization: str, company_id: str | None = None, fleet_id: str | None = None):
        query = {}
        if company_id:
        ...
        payload = self._request_json(
            url=self._build_url(settings.SETTLEMENT_PAYROLL_BASE_URL, endpoint),
    ...
    def list_settlement_items(self, *, authorization: str, company_id: str | None = None, fleet_id: str | None = None):
        query = {}
        if company_id:
            query["company_id"] = company_id
        if fleet_id:
    ...
    def list_drivers(
        ...
        *,
        ...
        if not isinstance(payload, dict) or not isinstance(payload.get("results"), list):
            raise SourceServiceError("Upstream request failed: malformed driver list payload.")
        return payload
    ...
    def list_driver_daily_settlements(self, *, driver_id: str, date_from: str, date_to: str, authorization: str):
        payload = self._request_json(
            url=self._build_url(
                settings.SETTLEMENT_PAYROLL_BASE_URL,
        ...
        if not isinstance(payload, dict):
            raise SourceServiceError("Upstream request failed: malformed driver daily settlement payload.")
        return payload

    def list_driver_daily_snapshots(self, *, driver_id: str, date_from: str, date_to: str, authorization: str):
        payload = self._request_json(
            url=self._build_url(
                settings.DELIVERY_RECORD_BASE_URL,
                f"/daily-snapshots/?{urlencode({'driver_id': driver_id, 'status': 'active'})}",
            ),
            authorization=authorization,
        )
        if not isinstance(payload, list):
            raise SourceServiceError("Upstream request failed: malformed delivery snapshot payload.")
        return [
            snapshot
            for snapshot in payload
            if isinstance(snapshot, dict)
            and date_from <= str(snapshot.get("service_date", "")) <= date_to
        ]
