...
The Dockerfile must not ship with `CMD runserver` for production. In the runtime container path, that bypasses the default branch in `entrypoint.sh`, so migrations never run and gunicorn never starts.
